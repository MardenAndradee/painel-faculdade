...
Nenhuma camada de negócio sabe onde os bytes moram. `StorageProvider` define `save`/`createReadStream`/`remove`; hoje existe só o `LocalStorageProvider`, que grava em `UPLOAD_DIR`. Trocar por S3, R2 ou GCS é escrever uma nova implementação — service, controller e rota ficam intactos.
