...
| 4 | Turmas que já existem hoje (pré-migração) recebem qual período? | **1, editável depois.** Não tem como inferir retroativamente em qual período do curso uma turma antiga estava. O dono corrige manualmente uma vez, se quiser, editando a turma. |
...
| R5 | Dono confunde "Finalizar semestre" com uma ação reversível e clica sem querer | Diálogo de confirmação explícito antes, listando o que vai acontecer (novo período, novo semestre, disciplinas/provas atuais saem da visão principal) — mesmo padrão do `CloseSemesterDialog` pessoal que já existe |
...
**Objetivo.** Garantir que cada usuário tenha, no máximo, uma participação ativa (`ClassMember.status: ACTIVE`) em turmas não arquivadas por vez — dono ou membro comum, é a mesma regra. Fecha uma ambiguidade que ficaria pior a partir da Etapa 30.2 em diante: não faz sentido um usuário estar "em dois períodos do curso ao mesmo tempo".
...
### Etapa 30.2 — Banco: Turma referencia Semestre de verdade
...
**Backend.** `class.service.create`: em vez de gravar `year`/`term` brutos, recebe `semesterId` (já resolvido/criado pelo frontend, ver 30.5) + `period`, grava direto. `class.service.addSubject` e `ensureMemberSubjectLink`: `ClassSubject`/`ClassPost` novos herdam `semesterId = class.semesterId` no momento da criação (nunca recalculado depois). `resolveMemberSemester` não muda de assinatura — continua resolvendo o semestre pessoal de qualquer membro a partir de `(year, term)`, só passa a ser alimentado por `class.semester.year`/`.term` em vez do par solto.
...
### Etapa 30.3 — Unificar sugestão de semestre e tirar a ambiguidade de "Período"
...
**Aceite.** Criar um semestre pela tela de Histórico e criar um semestre pelo atalho novo de dentro da Turma (Etapa 30.5) sugerem exatamente o mesmo nome/ano/período/datas pra a mesma data de hoje. Em nenhuma tela do app a palavra "Período" aparece mais se referindo à metade do ano civil.
...
### Etapa 30.4 — Backend: ação "Finalizar semestre" da turma

**Objetivo.** Endpoint novo que avança a turma pro próximo semestre/período automaticamente, arquivando o ciclo atual sem apagar nada.

**Backend.** `POST /classes/:id/finish-semester`, só o dono (`requireOwner`, mesmo guard já usado em `addSubject`). Sequência:
1. A partir do `Semester` atual da turma, calcula o próximo `(year, term)`: `term 1 → term 2` (mesmo ano); `term 2 → term 1` (ano seguinte).
2. Roda `resolveMemberSemester` pro **dono** nesse próximo `(year, term)` — pode reaproveitar um `Semester` pessoal que ele já tenha (ex.: já criou manualmente em Histórico) ou criar um novo pela regra da Etapa 30.3.
...
**Aceite.** Depois de finalizar, `GET /classes/:id` devolve o novo `semesterId`/`period`; toda disciplina/publicação criada depois pertence ao novo ciclo; nada do ciclo antigo é apagado ou alterado.

**Testes.** Teste de integração cobrindo: virada dentro do mesmo ano (term 1→2) e virada de ano (term 2→1, year+1); turma com múltiplos membros recebendo `semesterId` pessoal correto cada um; chamada por quem não é dono é rejeitada.
...
### Etapa 30.5 — Frontend: criação de Turma com Semestre real + Período do curso
...
### Etapa 30.6 — Frontend: "Finalizar semestre" na Turma + Histórico da turma

**Objetivo.** Dono ganha a ação de finalizar semestre na tela da turma; a turma passa a mostrar só o conteúdo do período atual por padrão, com o conteúdo antigo acessível numa aba própria.

**Frontend.**
- Botão "Finalizar semestre" (só visível pro dono), abrindo um diálogo de confirmação explicando o que vai acontecer (mesmo tom do `CloseSemesterDialog` pessoal já existente): "Vai avançar a turma para {próximo semestre sugerido}, {próximo período}º período. Disciplinas e publicações atuais saem da visão principal e ficam disponíveis no Histórico da turma."
- As abas/listas de Disciplinas e Publicações da turma passam a filtrar por `semesterId === turma.semesterId` (o atual) por padrão.
- Nova aba **Histórico** dentro da turma, listando os períodos anteriores (agrupados por `period`/`semesterId`), somente leitura.

**Aceite.** Depois de finalizar, a aba principal de Disciplinas/Publicações não mostra mais nada do período anterior; tudo daquele período continua acessível e intacto na aba Histórico da turma.

**Testes.** Verificação manual do fluxo completo (finalizar → conteúdo antigo sai da visão principal → aparece no Histórico da turma → dono adiciona disciplina nova → aparece só na visão principal do período novo).

---

### Etapa 30.7 — Frontend: cards de Prova/Publicações no padrão do projeto
...
- Isso já nasce escopado ao "período atual" pela Etapa 30.6 — sem esse filtro, a lista tenderia a crescer sem limite ao longo dos semestres.
